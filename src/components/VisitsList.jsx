    ...
    if (!error && data) {
      ...
      if (userIds.length > 0) {
        ...
        const visitsWithPractitioner = data.map(visit => ({
          ...visit,
          practitioner_name: profilesMap[visit.user_id] || 'Unknown'
        }))
        ...
        <div style={styles.cardBody}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={styles.table}>
              <thead>
                <tr>
                  ...
                  <th style={styles.th} onClick={() => handleSort('treatment')}>
                    Treatment {sortField === 'treatment' && (sortDirection === 'asc' ? '↑' : '↓')}
                  </th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan="5" style={styles.emptyRow}>Loading...</td>
                  </tr>
                ) : paginatedVisits.length === 0 ? (
                  <tr>
                    <td colSpan="5" style={styles.emptyRow}>No visits for this patient</td>
                  </tr>
                ) : (
          ...
          <div style={styles.pagination}>
            <div style={styles.pageInfo}>
              Showing {sortedVisits.length === 0 ? 0 : startIndex + 1} to {Math.min(endIndex, sortedVisits.length)} of {sortedVisits.length} entries
            </div>
        ...
        <div style={styles.modalOverlay} onClick={() => setShowCreateModal(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h3 style={styles.modalTitle}>Create Visit</h3>
              <button style={styles.modalClose} onClick={() => setShowCreateModal(false)}>
                <Icons.X />
              </button>
            </div>
            <div style={styles.modalBody}>
              <p>Are you sure you would like to create a new visit for this patient?</p>
            </div>
